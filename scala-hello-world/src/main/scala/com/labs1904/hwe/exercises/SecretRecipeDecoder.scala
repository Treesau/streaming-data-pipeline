...
object SecretRecipeDecoder {
  ...
  def decodeString(str: String): String = {
    //str.flatMap{x => if(x == ' ') ' '.toString else ENCODING.filter(_._2 == x.toString).keys.head}
    str.flatMap{x => if(x == ' ') ' '.toString else ENCODING(x.toString)}
  }
  ...
  def decodeIngredient(line: String): Ingredient = {
    // todo: implement me
    val ingredientList = line.split('#').map(decodeString)
    println(ingredientList(0), ingredientList(1))
    Ingredient(ingredientList(0), ingredientList(1))
  }
