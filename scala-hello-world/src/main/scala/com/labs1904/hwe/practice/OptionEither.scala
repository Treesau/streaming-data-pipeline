...
object OptionEither {
  ...
  */
  def totalCost(item: Item): Option[Double] =  {
    if (item.price.isDefined) Some(item.price.get + item.price.get * .07) else None
  }
  ...
  def averageTemperature(temperatures: List[WeatherStation]): Option[Int] = {
    val temps = temperatures.map(_.temperature).filter(_.isDefined).flatten
    if (temps.isEmpty) None else Some(temps.sum / temps.size)
  }
}
